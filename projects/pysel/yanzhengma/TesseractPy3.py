# -*- coding: utf-8 -*-
# @Time     : 2018/10/29 9:40
# @Author   : Philly
# @Site     : 
# @File     : TesseractPy3.py
...
def image_to_scratch(image, TEMP_IMAGE_NAME):
    # 将图片处理为兼容格式
    image.save(TEMP_IMAGE_NAME, dpi=(400, 400))
...
def call_tesseract(image, result, lang):
    # 调用 tesseract.exe, 将识别结果写入 output_filename 中
...
def image_to_string(image, lang, cleanup = CLEANUP_TEMP_FLAG, incompatible = INCOMPATIBLE):
    # 假如图片是不兼容的格式并且 incompatible = True, 先转换图片为兼容格式（本程序将图片转换为 .bmp 格式），
    # 然后获取识读结果；如果 cleanup = True，操作之后删除临时文件。
